...
%     data_name = 'isoL505030nsim1n12000_gauss_stddev5.mat';
    ...
    path_pics = '/home/ftelschow/PeakDetection/PeakDetection/pics/';
...
transformT2Z = 0;      % Option to transform the T field to a Z field before applying peak detection
...
if ~strcmp( fieldTYPE, 'T')
    ...
    [~,~,Rest] = spm_est_smoothness( char(names), path_mask, [3000 3000]);
...
for uu = 1:length(Uvec)
    ...
    %                 [~,~,R] = spm_est_smoothness( char(names), path_mask, [N N]);
        ...
        output_name = strcat('FieldTYPE_',fieldTYPE,'_N',int2str(N),'Msim',int2str(Msim),'_isotropicGauss', num2str(stddev(1)), num2str(stddev(2)), num2str(stddev(3)),'kappa',num2str(kappa),'_prethresh',num2str(ui),'_transform', num2str(transformT2Z));
...
if ~strcmp(machine,'server')
   ...
   %%
    ...
    % file ='FieldTYPE_Z_N50Msim10000_isotropicGauss555kappa1_prethresh2.5';
    % file ='FieldTYPE_T_N50Msim10000_isotropicGauss777kappa1_prethresh2.5';% output_name;
    % file ='FieldTYPE_T_N200Msim10000_isotropicGauss777kappa1_prethresh2.5_transform0';% output_name;
    file ='FieldTYPE_Z_N20Msim10000_isotropicGauss777kappa1_prethresh3_transform0';% output_name;
    load(strcat(path_sim,file,'.mat'))
    FieldTYPE = 'Z';%  'T'; %'T2Z'; % 'T2Z'; % choose 'T2Z' if transform1==1
...
if strcmp(FieldTYPE, 'Z') || strcmp(FieldTYPE, 'T2Z')
    ...
    if strcmp(FieldTYPE, 'Z')
        ...
        legend('GKF Ratio', 'Exact', 'Adler (1981)', 'GKF-1 Ratio', 'Uniform', 'Location', 'southeast' );
    elseif strcmp(FieldTYPE, 'T2Z')
        %legend( 'SPM12 (Gaussian)', 'Exact (Gaussian)', 'Chumbley (Gaussian)','Uniform', 'Location', 'southeast' );
        legend( 'GKF Ratio (Gaussian)', 'Exact (Gaussian)', 'GKF-1 Ratio (Gaussian)','Uniform', 'Location', 'southeast' );
        set(legend, 'fontsize', 20);
    else
        %legend( 'Exact (Gaussian)', 'SPM12 (t-stat)','Chumbley (t-stat)','Uniform', 'Location', 'southeast' );
        legend( 'Exact (Gaussian)', 'GKF Ratio (t-stat)','GKF-1 Ratio (t-stat)','Uniform', 'Location', 'southeast' );
    end
    if logPlot
