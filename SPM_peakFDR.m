function [Ps, Loc, u] = SPM_peakFDR(q, df, STAT, R, n, Z, ui, Loc)
%%%
%%% This function is only a slight modification of the function spm_uc_peakFDR
%%% from SPM12. We only changed the part of finding the peaks to make it
%%% compatible and comparable with the rest of our code.
%%% The section which we changed are indicated with comments.
%%%
% Peak False Discovery critical height threshold
% FORMAT [Ps, Loc] = spm_uc_peakFDR(q,df,STAT,R,n,Z,XYZ,ui[,G])
%
...
% % % % % START These are parts of the SPM code we do not need % % % % % %  
...
% end
% % % % % %  End These are parts of the SPM code we do not need % % % % % %

%%%%%%%%%%%% START this is our replacement %%%%%%%%%%%%
% find local maxima of the field and store them in Z
if nargin < 8
    Maxima   = zeros(size(Z));
    Imax     = imregionalmax(Z);
    Maxima( 2:(end-1), 2:(end-1), 2:(end-1)) = Imax( 2:(end-1), 2:(end-1), 2:(end-1));
    Z        = Z(logical(Maxima));
    I        = find(Z >= ui);
    Z        = Z(I);
...
%--------------------------------------------------------------------------
[~,~,Eu] = spm_P_RF(1,0,ui,df,STAT,R,n);
